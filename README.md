...
- Notebook support for inspecting the Q-table and plotting rewards
...
- `q_table.npy` (saved in the current working directory)
...
- If you run the notebook from `notebook/`, add the project root to `sys.path` before importing from `train.py`.
...
- `visualize.py` expects `q_table.npy` to exist first (run `python train.py` before visualization).
